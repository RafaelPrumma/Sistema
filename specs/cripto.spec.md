...
- **F2 — Valoração + baldes + TOTAL correto (prioridade):** preço BRL na data p/ permutas; baldes Trade/Rendimentos; alertas de valor faltante. **⚠️ Crítico (jun/2026): o BTC vem subcontado** — a maior parte do BTC do usuário está no **Earn** (Simple Earn), e o netting F1 erra por (a) o vai-e-volta diário Subscription/Redemption do Earn (churn — não fecham) e (b) usar **só o ledger**, perdendo trades do arquivo Spot. Refazer pra: **Spot+Convert+ledger combinados** (dedup, sem perder trade); **Earn/Staking conta como detido** (o saldo parado no Earn entra na posição). Alternativa rápida: **reconciliar pelos saldos reais por moeda** (mesmo mecanismo VARIAÇÃO da B3 — `importador-b3.spec.md` §10.4) já que não há "Posição" de cripto importada. Ref.: BTC Earn real ≈ 0,054.
