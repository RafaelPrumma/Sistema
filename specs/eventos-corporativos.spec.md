...
- **F3 — Auto-busca Brapi: ✅ feito (jun/2026).** `AtualizarEventosCorporativosAsync` lê `dividendsData.stockDividends` da Brapi (espelha o job de proventos); `EventoCorporativoBrapiParser` (puro) mapeia `label`→Tipo, `factor`→Fator (invertido p/ grupamento), `lastDatePrior`+1=data-ex; **dedup por janela** (ticker + ~7 dias) p/ não duplicar os semeados/manuais; chave via `GerarChaveNatural`. Job `financas-eventos-corporativos` (`Cron.Daily`). Sem token → loga p/ cadastro manual. Bonificação não é auto-inserida (fator ambíguo). 6 testes, `dotnet test` verde (60/60). Sem migration. **Eventos corporativos = feature completa (F1+F2+F3).**
